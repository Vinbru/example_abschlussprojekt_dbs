...
public class SQLiteUserRepository implements UserRepository {
    ...
    public Optional<User> findByName(String name) {
        ...
        try (Connection connection = dataSource.getConnection()) {
            String sql =    "SELECT * \n" +
                            "FROM (\n" +
                                "SELECT [email], 'KUNDE' FROM KUNDE UNION ALL \n" +
                                "SELECT [email], 'BIBLIOTHEKAR' FROM BIBLIOTHEKAR UNION ALL\n" +
                                "SELECT [email], 'NUTZER' FROM NUTZER)\n" +
                            "WHERE [email] LIKE ?;";
            // TODO: Dem Benutzer 'name' die entsprechenden Berechtigungen geben (siehe enum Role), um diese per RolesAllowed-Annotation zu nutzen.
    ...
    public long countByNameAndPassword(String name, String password) {
        try (Connection connection = dataSource.getConnection()) {
            ...
            // TODO: Die Anzahl der Benutzer mit 'name' als E-Mail-Adresse bzw. Benutzernamen und 'password' als Passwort zurueckgeben.
            PreparedStatement preparedStatement = connection.prepareStatement(sql);
            preparedStatement.closeOnCompletion();
            preparedStatement.setObject(1, name);
